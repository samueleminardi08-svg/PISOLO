...
// ATTENZIONE: La chiave Ã¨ visibile a chiunque ispezioni il codice sorgente.
...
const ALTERNATING_MASCOT_PRESETS = [
  { type: 'shark', id: 'shark_1', title: 'Pisolo Morbido ðŸ¦ˆ', desc: 'Squaletto soffice e rassicurante', tag: 'ðŸ¦ˆ Squaletto #1', badge: 'ðŸ¦ˆ Pisolo â€¢ Foto 1/4' },
  { type: 'kitty', id: 'kitty_1', title: 'Micio Dolce ðŸ±', desc: 'Gattino coccoloso per le tue giornate', tag: 'ðŸ± Gattino #1', badge: 'ðŸ± Micio â€¢ Foto 1/4' },
  { type: 'shark', id: 'shark_2', title: 'Pisolo Nuotatore ðŸ¦ˆ', desc: 'Pronto a farti da scudo in ogni tempesta', tag: 'ðŸ¦ˆ Squaletto #2', badge: 'ðŸ¦ˆ Pisolo â€¢ Foto 2/4' },
  { type: 'kitty', id: 'kitty_2', title: 'Micio Sonnecchiante ðŸ±', desc: 'Fusa rilassanti e nanna tranquilla', tag: 'ðŸ± Gattino #2', badge: 'ðŸ± Micio â€¢ Foto 2/4' },
  { type: 'shark', id: 'shark_3', title: 'Pisolo Cucciolotto ðŸ¦ˆ', desc: 'Tenerezza infinita per scaldarti il cuore', tag: 'ðŸ¦ˆ Squaletto #3', badge: 'ðŸ¦ˆ Pisolo â€¢ Foto 3/4' },
  { type: 'kitty', id: 'kitty_3', title: 'Micio Curioso ðŸ±', desc: 'Occhietti dolci che ti sorridono', tag: 'ðŸ± Gattino #3', badge: 'ðŸ± Micio â€¢ Foto 3/4' },
  { type: 'shark', id: 'shark_4', title: 'Pisolo Protettivo ðŸ¦ˆ', desc: 'Un abbraccio grande a pinne aperte', tag: 'ðŸ¦ˆ Squaletto #4', badge: 'ðŸ¦ˆ Pisolo â€¢ Foto 4/4' },
  { type: 'kitty', id: 'kitty_4', title: 'Micio e Amico ðŸ±', desc: "Coccole senza fine e fusa d'amore", tag: 'ðŸ± Gattino #4', badge: 'ðŸ± Micio â€¢ Foto 4/4' }
];
...
function getAlternatingPhoto(index) {
  ...
  return {
    ...
    desc: preset.desc,
    tag: preset.tag,
    badge: isShark ? `ðŸ¦ˆ Pisolo â€¢ ${preset.title}` : `ðŸ± Micio â€¢ ${preset.title}`,
    mascotName: isShark ? "Pisolo ðŸ¦ˆ" : "Micio ðŸ±"
  };
}
...
function getActiveSharkTitle() {
  ...
  if (preset) return preset.title + ' ðŸ¦ˆ';
  return localStorage.getItem('mascot_shark_selected_title') || "Pisolo Classico ðŸ¦ˆ";
}
...
function getActiveKittyTitle() {
  ...
  if (preset) return preset.title + ' ðŸ±';
  return localStorage.getItem('mascot_kitty_selected_title') || "Micio Dolce ðŸ±";
}
...
const MASCOT_QUOTES = {
  shark: [
    ...
    "Anche nei giorni piÃ¹ difficili, la tua forza mi rende fiero di te."
  ],
  kitty: [
    ...
    "Sei la persona piÃ¹ dolce che esista. Non dimenticarlo mai."
  ]
};
...
// ============================================================
// NOTA IMPORTANTE: la password non Ã¨ piÃ¹ salvata in chiaro nel codice.
// Viene confrontata tramite hash SHA-256, quindi chi apre il "Visualizza
// sorgente" o la console del browser non la trova piÃ¹ leggibile.
// Attenzione perÃ²: questa resta comunque una protezione "leggera", adatta
...
async function handleLoginSubmit(e) {
  ...
  if (inputHash === APP_PASSWORD_HASH) {
    ...
    showToast("Accesso eseguito! Benvenuta nel tuo rifugio sicuro â¤ï¸");
...
function selectMascot(type) {
  ...
  updateMascotVisual();
  showToast(type === 'shark' ? "Pisolo lo squaletto ti fa compagnia ðŸ¦ˆ" : "Micio il gattino ti fa le fusa ðŸ±");
}
...
function nextDelicateQuote() {
  ...
  showToast("Un pensiero dolce da " + (currentMascot === 'shark' ? "Pisolo ðŸ¦ˆ" : "Micio ðŸ±"));
}
...
function updateMascotVisual() {
  ...
  if (currentMascot === 'shark') {
    ...
    if (badgeName) badgeName.textContent = 'Pisolo ðŸ¦ˆ';
    if (pillShark) pillShark.classList.add('active');
    if (pillKitty) pillKitty.classList.remove('active');
    if (speechIcon) speechIcon.textContent = 'ðŸ¦ˆ';
    if (partnerIcon) partnerIcon.textContent = 'ðŸ¦ˆ';
  } else {
    if (imgEl) imgEl.src = currentKittySrc;
    if (badgeName) badgeName.textContent = 'Micio ðŸ±';
    if (pillKitty) pillKitty.classList.add('active');
    if (pillShark) pillShark.classList.remove('active');
    if (speechIcon) speechIcon.textContent = 'ðŸ±';
    if (partnerIcon) partnerIcon.textContent = 'ðŸ±';
  }
  displayCurrentMascotQuote();
}
...
function displayCurrentMascotQuote() {
  ...
  if (textEl) {
    textEl.textContent = `â€œ${randQuote}â€`;
  }
}
...
const SUPPORT_MESSAGES = [
  {
    id: 1,
    categoria: "tristezza",
    tag: "ðŸ«‚ Vicinanza sincera",
    title: "Non sei mai sola",
    titolo: "Non sei mai sola",
    testo: "So che ci sono momenti in cui tutto sembra pesare un po' di piÃ¹ e trovare le energie Ã¨ faticoso. Ma tu non sei sola: io sono qui, silenziosamente al tuo fianco a sostenerti.",
    autore: "â€” Chi ti vuole un bene infinito",
    firma: "â€” Chi ti vuole un bene infinito"
  },
  {
    id: 2,
    categoria: "ansia",
    tag: "ðŸŒ¸ Dolcezza pura",
    title: "Prenditi il tuo tempo",
    titolo: "Prenditi il tuo tempo",
    testo: "Non devi essere per forza invincibile ogni singolo giorno. Puoi sentirti fragile, puoi respirare e concederti di staccare. Sei amatissima proprio cosÃ¬ come sei.",
    autore: "â€” Il tuo porto sicuro",
    firma: "â€” Il tuo porto sicuro"
  },
  {
    id: 3,
    categoria: "tristezza",
    tag: "ðŸ›¡ï¸ Protezione e calore",
    title: "Il tuo rifugio sicuro",
    titolo: "Il tuo rifugio sicuro",
    testo: "Qualsiasi pensiero ti stia togliendo serenitÃ  in questo momento, lascialo andare per qualche minuto. Chiudi gli occhi: ci sono le mie braccia a fare da scudo a tutto.",
    autore: "â€” Per sempre con te",
    firma: "â€” Per sempre con te"
  },
  {
    id: 4,
    categoria: "generale",
    ...
    testo: "Sei la persona piÃ¹ speciale e generosa che conosca. Anche quando la giornata Ã¨ scura, ricordati che porti una luce immensa nella mia vita.",
    autore: "â€” Il tuo fan numero uno",
    firma: "â€” Il tuo fan numero uno"
  },
  {
    id: 5,
    categoria: "ansia",
    tag: "ðŸŒŠ Calma e quiete",
    ...
    autore: "â€” Pisolo ðŸ¦ˆ & Micio ðŸ±",
    firma: "â€” Pisolo ðŸ¦ˆ & Micio ðŸ±"
  },
  {
    id: 6,
    categoria: "generale",
    tag: "â˜€ï¸ Raggio di sole",
    title: "La parte piÃ¹ bella di me",
    titolo: "La parte piÃ¹ bella di me",
    testo: "Sei il primo pensiero bello della mattina e il rifugio della sera. Quando sorridi, il mondo intorno torna a brillare.",
    autore: "â€” Sempre al tuo fianco",
    firma: "â€” Sempre al tuo fianco"
  },
  {
    id: 7,
    categoria: "fisico",
    tag: "ðŸ§¸ Coccole infinite",
    ...
    autore: "â€” Con tutto il cuore",
    firma: "â€” Con tutto il cuore"
  },
  {
    id: 8,
    categoria: "generale",
    tag: "ðŸ’« Un patto tra noi",
    title: "Io non vado da nessuna parte",
    titolo: "Io non vado da nessuna parte",
    testo: "Nessun malumore, nessuna stanchezza e nessuna distanza potrÃ  mai cambiare quello che provo per te. Per me sei casa.",
    autore: "â€” Il tuo ragazzo â¤ï¸",
    firma: "â€” Il tuo ragazzo â¤ï¸"
  },
  {
    id: 9,
    categoria: "ansia",
    tag: "ðŸƒ Lascia scorrere",
    ...
    autore: "â€” Chi ti tiene la mano",
    firma: "â€” Chi ti tiene la mano"
  },
  {
    id: 10,
    categoria: "fisico",
    tag: "â˜• Calore rilassante",
    title: "Un momento tutto tuo",
    titolo: "Un momento tutto tuo",
    testo: "Se il corpo Ã¨ indolenzito o stanco, concediti una tazza calda e una pausa senza sensi di colpa. La salute e la tua serenitÃ  vengono prima di qualunque dovere.",
    autore: "â€” Il tuo alleato speciale",
    firma: "â€” Il tuo alleato speciale"
  },
  {
    id: 11,
    categoria: "tristezza",
    tag: "ðŸŒ§ï¸ Anche la pioggia passa",
    title: "Puoi piangere se serve",
    titolo: "Puoi piangere se serve",
    testo: "Le lacrime non sono debolezza: lavano via la polvere dalle giornate pesanti. Piangi pure se ne senti il bisogno, io sarÃ² qui ad asciugarti il viso con dolcezza.",
    autore: "â€” Il tuo abbraccio sicuro",
    firma: "â€” Il tuo abbraccio sicuro"
  },
  {
    id: 12,
    categoria: "generale",
    tag: "â­ Il mio orgoglio",
    title: "Sei fortissima, ricordalo",
    titolo: "Sei fortissima, ricordalo",
    testo: "Hai superato ogni singolo giorno difficile che hai incontrato nella tua vita finora. Ce la farai anche questa volta, e io sarÃ² lÃ¬ a festeggiare ogni tuo piccolo passo.",
    autore: "â€” Chi crede in te al 100%",
    firma: "â€” Chi crede in te al 100%"
  },
  {
    id: 13,
    categoria: "ansia",
    tag: "âš“ Ancora sicura",
    title: "Guarda qui: siamo al sicuro",
    titolo: "Guarda qui: siamo al sicuro",
    testo: "Quando il cuore batte troppo in fretta, appoggia la mano sul petto. Fai cinque respiri lenti insieme a me. Niente di brutto puÃ² raggiungerti in questo istante.",
    autore: "â€” Pisolo ðŸ¦ˆ il tuo custode",
    firma: "â€” Pisolo ðŸ¦ˆ il tuo custode"
  },
  {
    id: 14,
    categoria: "fisico",
    tag: "ðŸ›‹ï¸ Riposo totale",
    title: "Fermati e rilassati",
    titolo: "Fermati e rilassati",
    testo: "Micio ti invita a fare come lui: trova la posizione piÃ¹ morbida del letto, sciogli le spalle e chiudi gli occhi. Il mondo lÃ  fuori puÃ² benissimo aspettare.",
    autore: "â€” Micio ðŸ± tuo complice",
    firma: "â€” Micio ðŸ± tuo complice"
  },
  {
    id: 15,
    categoria: "tristezza",
    ...
    testo: "Se qualcuno o qualcosa ti ha ferita oggi, lascia che il mio affetto rimetta a posto i pezzi. Per me sei il tesoro piÃ¹ grande del mondo.",
    autore: "â€” Il tuo rifugio permanente",
    firma: "â€” Il tuo rifugio permanente"
  },
  {
    id: 16,
    categoria: "generale",
    tag: "ðŸ’Œ Amore senza fine",
    title: "Mi manchi sempre",
    titolo: "Mi manchi sempre",
    testo: "Non c'Ã¨ ora del giorno in cui io non pensi a quanto sono fortunato ad averti accanto. Ti stringo fortissimo anche attraverso lo schermo.",
    autore: "â€” Il tuo ragazzo â¤ï¸",
    firma: "â€” Il tuo ragazzo â¤ï¸"
  },
  {
    ...
    autore: "â€” Il tuo scudo contro l'ansia",
    firma: "â€” Il tuo scudo contro l'ansia"
  },
  {
    id: 18,
    categoria: "fisico",
    tag: "ðŸ«– Tisana e coperta",
    ...
    autore: "â€” La tua farmacia di coccole",
    firma: "â€” La tua farmacia di coccole"
  },
  {
    id: 19,
    categoria: "tristezza",
    tag: "ðŸ’– Senza condizioni",
    ...
    autore: "â€” Chi non ti lascerÃ  mai",
    firma: "â€” Chi non ti lascerÃ  mai"
  },
  {
    id: 20,
    categoria: "generale",
    tag: "ðŸŒ™ Buonanotte al cuore",
    title: "Lascia andare la giornata",
    titolo: "Lascia andare la giornata",
    testo: "Qualunque cosa sia successa oggi, ormai Ã¨ passata. Sei qui, sei al sicuro e domani sarÃ  una pagina tutta nuova da scrivere insieme con calma.",
    autore: "â€” I tuoi sogni custoditi",
    firma: "â€” I tuoi sogni custoditi"
  },
  {
    id: 21,
    categoria: "tristezza",
    tag: "ðŸ›¡ï¸ Nessuna paura",
    title: "Ci penso io a farti da scudo",
    titolo: "Ci penso io a farti da scudo",
    testo: "Se le parole o le persone fuori ti hanno ferita o stancata, chiudi quella porta. Qui dentro ci siamo solo noi due, e nessuno puÃ² farti del male.",
    autore: "â€” Il tuo cavaliere guardiano",
    firma: "â€” Il tuo cavaliere guardiano"
  },
  {
    id: 22,
    categoria: "ansia",
    ...
    title: "Nuota a riva",
    titolo: "Nuota a riva",
    testo: "Quando ti sembra di affogare nelle cose da fare, ricordati che Pisolo nuota sotto di te per sostenerti. Non puoi affondare finchÃ© ci sono io.",
    autore: "â€” Pisolo ðŸ¦ˆ il tuo custode",
    firma: "â€” Pisolo ðŸ¦ˆ il tuo custode"
  },
  {
    id: 23,
    categoria: "generale",
    tag: "ðŸŒ¸ Tenerezza infinita",
    title: "Una carezza per il tuo viso",
    titolo: "Una carezza per il tuo viso",
    testo: "Vorrei essere lÃ¬ a sfiorarti le guance, a baciarti la fronte e a sussurrarti all'orecchio quanto sei preziosa per me. Immagina il mio bacio adesso.",
    autore: "â€” Il tuo amore grande",
    firma: "â€” Il tuo amore grande"
  },
  {
    id: 24,
    categoria: "fisico",
    tag: "ðŸ©¹ Cura e pazienza",
    ...
    autore: "â€” Chi ti aspetta sempre",
    firma: "â€” Chi ti aspetta sempre"
  },
  {
    id: 25,
    categoria: "ansia",
    tag: "ðŸŽˆ Leggera come una piuma",
    title: "Meno severa con te stessa",
    titolo: "Meno severa con te stessa",
    testo: "Sei sempre cosÃ¬ attenta a tutti gli altri. Per una volta, sii dolce e comprensiva con te stessa come lo saresti con la persona a cui vuoi piÃ¹ bene.",
    autore: "â€” Il tuo specchio sincero",
    firma: "â€” Il tuo specchio sincero"
  },
  {
    id: 26,
    categoria: "fisico",
    tag: "ðŸ« Dolcezza riparatrice",
    ...
    autore: "â€” Chi vuole vederti serena",
    firma: "â€” Chi vuole vederti serena"
  },
  {
    id: 27,
    categoria: "tristezza",
    tag: "ðŸ«‚ Braccia aperte",
  ...
  },
  {
    ...
    testo: "Micio si raggomitola accanto a te, fa le fusa al ritmo del tuo respiro e ti ricorda che i gatti sanno sempre quando Ã¨ il momento di non fare proprio nulla.",
    autore: "â€” Micio ðŸ± tuo complice",
    firma: "â€” Micio ðŸ± tuo complice"
  },
  {
    ...
    testo: "Non cambiare mai la tua sensibilitÃ : Ã¨ la cosa piÃ¹ preziosa e rara che esista. Anche quando il mondo sembra ruvido, la tua dolcezza vincerÃ  sempre.",
    autore: "â€” Il tuo ragazzo, per sempre â¤ï¸",
    firma: "â€” Il tuo ragazzo, per sempre â¤ï¸"
  },
  {
    id: 30,
    categoria: "ansia",
    tag: "ðŸŒˆ SerenitÃ  ritrovata",
    title: "La forza dentro di te",
    titolo: "La forza dentro di te",
    testo: "Dentro di te c'Ã¨ una calma profonda che nessuna fretta puÃ² cancellare. Chiudi gli occhi, fai spazio al silenzio e ritrova la tua pace.",
    autore: "â€” Chi crede sempre in te",
    firma: "â€” Chi crede sempre in te"
  },
  {
    id: 31,
    categoria: "tristezza",
    tag: "ðŸ•¯ï¸ Piccolo faro",
    ...
    autore: "â€” Il tuo faro nella notte",
    firma: "â€” Il tuo faro nella notte"
  },
  {
    id: 32,
    categoria: "generale",
    tag: "ðŸ’– Per sempre noi",
    ...
    autore: "â€” Pisolo ðŸ¦ˆ, Micio ðŸ± & Il tuo Amore â¤ï¸",
    firma: "â€” Pisolo ðŸ¦ˆ, Micio ðŸ± & Il tuo Amore â¤ï¸"
  }
];

// Alias per compatibilitÃ  con eventuale codice preesistente
...
function displayMessageOnCard(m) {
  ...
  if (quoteEl) quoteEl.textContent = `â€œ${m.testo}â€`;
...
function shuffleFeaturedMessage() {
  ...
  showToast("Nuovo pensiero e foto speciale per te ðŸ’Œ");
}
...
function handleHeroUpload(e) {
  ...
  compressImageFile(file, (b64) => {
    try {
      ...
      showToast("Foto copertina personalizzata aggiornata! ðŸ“¸");
    } catch (err) {
      showToast("Impossibile salvare la foto, prova con una piÃ¹ leggera.");
    }
  });
  e.target.value = '';
}
...
async function fetchAiHug(category) {
  ...
  const prompt = `Sei Pisolo ðŸ¦ˆ (squaletto) e Micio ðŸ± (gattino). Dai un abbraccio virtuale breve, rassicurante e unico (tema: ${mood}).${avoid} Scrivi solo la frase dell'abbraccio in italiano, senza asterischi (max 35 parole).`;
  ...
  if (generatedText) {
    ...
    return {
      testo: generatedText,
      autore: "Pisolo ðŸ¦ˆ & Micio ðŸ± (con IA âœ¨)",
      firma: "Pisolo ðŸ¦ˆ & Micio ðŸ± (con IA âœ¨)",
...
async function openHugModal(category = null) {
  ...
  if (userKey && !isAiHugGenerating) {
    if (modalBodyEl) modalBodyEl.innerHTML = '<span style="font-size:14px; color:#7E22CE; display:flex; align-items:center; justify-content:center; gap:8px;"><span style="animation:spinSmooth 1.5s infinite linear;">âœ¨</span> Pisolo e Micio ti stanno preparando una carezza...</span>';
...
async function cycleNextHugMessage() {
  ...
  let msg = null;
  if (userKey && !isAiHugGenerating) {
    if (modalBodyEl) modalBodyEl.innerHTML = '<span style="font-size:14px; color:#7E22CE; display:flex; align-items:center; justify-content:center; gap:8px;"><span style="animation:spinSmooth 1.5s infinite linear;">âœ¨</span> Stiamo preparando un nuovo abbraccio per te...</span>';
    isAiHugGenerating = true;
    const aiMsg = await fetchAiHug(activeHugCategory);
    isAiHugGenerating = false;
    if (aiMsg) {
  ...
  displayMessageInModal(msg);
  displayMessageOnCard(msg);
  showToast("Nuovo abbraccio speciale con foto per te â¤ï¸");
}
...
function thankYouHug() {
  for (let i = 0; i < 14; i++) {
    setTimeout(() => {
      ...
      h.textContent = ['ðŸ’–', 'ðŸ’•', 'ðŸŒ¸', 'âœ¨', 'ðŸ§¸'][Math.floor(Math.random() * 5)];
  ...
  document.getElementById('modal-hug-backdrop').style.display = 'none';
  showToast("Ti voglio un bene immenso â¤ï¸");
}

// ============================================================
// 6. SFONDI TRANQUILLI & LEGGERI CON LUOGHI DI PACE REALI (10 VARIANTI)
// ============================================================
const TRANQUIL_BACKGROUNDS = [{ "id": "spiaggia-serena", "name": "Spiaggia Serena", "emoji": "ðŸ–ï¸", "image": "assets/img_14_71f7b0643166.jpg", "place": "Spiaggia calma e mare turchese trasparente", "themeColor": "#EBF5FB" }, { "id": "boschetto-luce", "name": "Boschetto di Luce", "emoji": "ðŸŒ²", "image": "assets/img_15_938af34ab049.jpg", "place": "Bosco silenzioso con raggi di sole gentili", "themeColor": "#EAF8F1" }, { "id": "lago-quieto", "name": "Lago Quieto", "emoji": "ðŸžï¸", "image": "assets/img_16_599c09a8ed3b.jpg", "place": "Lago alpino immobile e trasparente all'aurora", "themeColor": "#F2EEF8" }, { "id": "campi-lavanda", "name": "Campi di Lavanda", "emoji": "ðŸª»", "image": "assets/img_17_016e937ee8b6.jpg", "place": "Distesa fiorita provenzale al tramonto", "themeColor": "#F5EFFB" }, { "id": "giardino-zen", "name": "Giardino Zen", "emoji": "ðŸŒ¸", "image": "assets/img_18_c11de8ca4c11.jpg", "place": "Fiori di ciliegio, specchio d'acqua e quiete", "themeColor": "#FDF0F4" }, { "id": "tramonto-mare", "name": "Tramonto sul Mare", "emoji": "ðŸŒ…", "image": "assets/img_19_2ad37fb1fbd2.jpg", "place": "Spiaggia dorata soffice e orizzonte sereno", "themeColor": "#FFF3E8" }, { "id": "prato-fiorito", "name": "Prato Fiorito", "emoji": "ðŸŒ¼", "image": "assets/img_20_a1294a4cb05c.jpg", "place": "Dolci colline verdi con margherite campestri", "themeColor": "#F4FAF2" }, { "id": "cascata-segreta", "name": "Cascata Segreta", "emoji": "ðŸ’§", "image": "assets/img_21_238c3b53e343.jpg", "place": "Ruscello limpido tra sassi levigati e muschio", "themeColor": "#EDF8F7" }, { "id": "colline-aurora", "name": "Colline all'Aurora", "emoji": "ðŸŒ„", "image": "assets/img_22_24b7db2b4e85.jpg", "place": "Vallata verde con foschia dorata mattutina", "themeColor": "#F7FBF4" }, { "id": "fiume-salici", "name": "Fiume dei Salici", "emoji": "ðŸŒ¿", "image": "assets/img_23_e58588a5599c.jpg", "place": "Ninfee e salici piangenti sull'acqua cheta", "themeColor": "#EEF7F5" }];
...
function cycleNextBackground() {
  ...
  showToast(`Luogo di pace: ${bg.emoji} ${bg.name} â¤ï¸`);
}
  ...
  TRANQUIL_BACKGROUNDS.forEach((bg, idx) => {
    ...
    item.innerHTML = `
          <div class="bg-picker-thumb" style="background-image: url('${bg.image}');">
            ...
            <div class="bg-picker-active-badge">In uso âœ“</div>
          </div>
...
// ============================================================
// GESTIONE INSTALLAZIONE PWA & MODALITÃ€ SCHERMO INTERO NATURA
// ============================================================

// ModalitÃ  Schermo Intero PWA / Browser
function toggleAppFullscreen() {
  const isStandalone = window.matchMedia('(display-mode: standalone)').matches || window.navigator.standalone === true;
  if (isStandalone) {
    showToast("Sei giÃ  in modalitÃ  Schermo Intero PWA! ðŸ’–");
    return;
  }
  if (!document.fullscreenElement && !document.webkitFullscreenElement) {
    if (document.documentElement.requestFullscreen) {
      document.documentElement.requestFullscreen().catch(() => {
        showToast("Visualizzazione espansa attiva ðŸŒ¸");
      });
    } else if (document.documentElement.webkitRequestFullscreen) {
      document.documentElement.webkitRequestFullscreen();
    } else {
      showToast("Visualizzazione a tutto schermo ðŸŒ¸");
    }
  } else {
...
const ALL_COMFORT_TIPS = [
  ...
  { id: 1, categoria: "sleep", emoji: "ðŸ«", titolo: "Fai un respiro profondo con Pisolo", descrizione: "Segui il ritmo calmo dell'esercizio guidato 4-4-4 per rallentare il battito e preparare il corpo al sonno.", isBreath: true },
  { id: 2, categoria: "sleep", emoji: "ðŸ›‹ï¸", titolo: "Trova la posizione perfetta", descrizione: "Rannicchiati sotto le coperte, abbraccia un cuscino e lascia che le spalle sprofondino nel materasso.", isBreath: false },
  { id: 3, categoria: "sleep", emoji: "ðŸŽ§", titolo: "Ascolta il suono della pioggia", descrizione: "Metti una traccia di pioggia leggera o onde del mare a basso volume, e concentrati solo su quel rumore costante.", isBreath: false },
  { id: 4, categoria: "sleep", emoji: "ðŸ•¯ï¸", titolo: "Visualizza un luogo tranquillo", descrizione: "Immagina di essere su una barca che dondola lentamente su un lago calmo sotto un cielo stellato.", isBreath: false },
  { id: 5, categoria: "sleep", emoji: "ðŸ™ˆ", titolo: "Rilassa gli occhi", descrizione: "Chiudi gli occhi e immagina uno spazio completamente nero, rilassando i muscoli della fronte e delle palpebre.", isBreath: false },
  { id: 6, categoria: "sleep", emoji: "ðŸ’†â€â™€ï¸", titolo: "Rilassamento muscolare progressivo", descrizione: "Parti dai piedi: tendili per due secondi e poi rilassali completamente. Sali fino al collo e al viso.", isBreath: false },

  // ANXIETY (Ansia o agitazione)
  { id: 7, categoria: "anxiety", emoji: "ðŸ’§", titolo: "Bevi un sorso d'acqua fresca", descrizione: "Sorseggia lentamente acqua fredda: aiuta a resettare il nervo vago e a calmare il battito cardiaco.", isBreath: false },
  { id: 8, categoria: "anxiety", emoji: "ðŸ«", titolo: "Respirazione a labbra socchiuse", descrizione: "Inspira piano dal naso ed espira lentamente dalla bocca, come se dovessi soffiare su una piuma.", isBreath: true },
  { id: 9, categoria: "anxiety", emoji: "ðŸ¦¶", titolo: "Senti il pavimento sotto di te", descrizione: "Concentrati sul contatto dei piedi con il suolo. Conta 5 cose che puoi vedere e 4 che puoi toccare.", isBreath: false },
  { id: 10, categoria: "anxiety", emoji: "ðŸ§Š", titolo: "Tocca qualcosa di freddo", descrizione: "Tieni in mano un cubetto di ghiaccio o lava i polsi e il viso con acqua fredda per interrompere l'agitazione.", isBreath: false },
  { id: 11, categoria: "anxiety", emoji: "ðŸ›¡ï¸", titolo: "DÃ¬ a te stessa: 'PasserÃ '", descrizione: "L'ansia Ã¨ come un'onda: ora Ã¨ alta, ma inevitabilmente si abbasserÃ . Sei al sicuro.", isBreath: false },
  { id: 12, categoria: "anxiety", emoji: "ðŸ«‚", titolo: "Stringi forte un cuscino", descrizione: "Applica una leggera pressione sul petto abbracciando un cuscino o coprendoti con una coperta pesante.", isBreath: false },

  // DISTANCE (Vi mancate a distanza)
  { id: 13, categoria: "distance", emoji: "ðŸ’Œ", titolo: "Scrivimi un messaggio dolce", descrizione: "Anche se non sono lÃ¬, leggerti mi fa sentire vicino. Raccontami una cosa bella di oggi.", isBreath: false },
  { id: 14, categoria: "distance", emoji: "ðŸ“¸", titolo: "Guarda una nostra foto speciale", descrizione: "Cerca nella galleria del telefono la foto di un momento in cui ridevamo insieme.", isBreath: false },
  { id: 15, categoria: "distance", emoji: "ðŸ§¸", titolo: "Stringi un peluche", descrizione: "Abbraccia Pisolo o Micio immaginando che sia io a tenerti stretta, forte forte.", isBreath: false },
  { id: 16, categoria: "distance", emoji: "â¤ï¸", titolo: "Siamo sotto lo stesso cielo", descrizione: "Guarda fuori dalla finestra: anche se lontani, guardiamo le stesse stelle e la stessa luna.", isBreath: false },
  { id: 17, categoria: "distance", emoji: "ðŸ—“ï¸", titolo: "Pensa al nostro prossimo incontro", descrizione: "Focalizzati su cosa faremo la prossima volta che ci vedremo. Il tempo passerÃ  in fretta.", isBreath: false },
  { id: 18, categoria: "distance", emoji: "ðŸŽ¶", titolo: "Ascolta la 'nostra' canzone", descrizione: "Metti in play quella canzone che ti fa pensare a noi e chiudi gli occhi per un istante.", isBreath: false },

  // HEAVY_DAY (Dopo una giornata pesante)
  { id: 19, categoria: "heavy_day", emoji: "â˜•", titolo: "Una bevanda calda tutta per te", descrizione: "Prepara un tÃ¨ o una cioccolata calda e stringi la tazza con entrambe le mani per assorbire il calore.", isBreath: false },
  { id: 20, categoria: "heavy_day", emoji: "ðŸ›", titolo: "Fai una doccia rilassante", descrizione: "Lascia scorrere l'acqua calda sulle spalle e immagina che lavi via tutto lo stress della giornata.", isBreath: false },
  { id: 21, categoria: "heavy_day", emoji: "ðŸ“±", titolo: "15 minuti di silenzio", descrizione: "Silenzia le notifiche e metti via il telefono. Per un quarto d'ora non devi essere produttiva.", isBreath: false },
  { id: 22, categoria: "heavy_day", emoji: "ðŸ§¦", titolo: "Mettiti comoda", descrizione: "Togliti i vestiti da lavoro/studio, indossa il pigiama piÃ¹ morbido che hai o dei calzini caldi.", isBreath: false },
  { id: 23, categoria: "heavy_day", emoji: "ðŸ«", titolo: "Concediti un piccolo premio", descrizione: "Gusta un pezzetto di cioccolato o il tuo snack preferito: te lo sei meritato oggi.", isBreath: false },
  { id: 24, categoria: "heavy_day", emoji: "ðŸ•Šï¸", titolo: "Perdona ciÃ² che non sei riuscita a fare", descrizione: "Hai fatto del tuo meglio. CiÃ² che Ã¨ rimasto indietro aspetterÃ  domani senza problemi.", isBreath: false },

  // ANGER (Quando sei arrabbiata)
  { id: 25, categoria: "anger", emoji: "âœï¸", titolo: "Scrivi tutto e buttalo via", descrizione: "Prendi un foglio, scrivi di getto tutto ciÃ² che ti fa rabbia, poi accartoccialo o strappalo.", isBreath: false },
  { id: 26, categoria: "anger", emoji: "ðŸš¶â€â™€ï¸", titolo: "Fai due passi fuori", descrizione: "Esci di casa anche solo per 5 minuti. L'aria fresca sul viso aiuta a dissipare l'energia trattenuta.", isBreath: false },
  { id: 27, categoria: "anger", emoji: "ðŸ«", titolo: "Respira profondamente", descrizione: "Usa la funzione di respiro qui nell'app. Concentrati solo sul conteggio per spezzare il pensiero.", isBreath: true },
  { id: 28, categoria: "anger", emoji: "ðŸ’§", titolo: "Lavati il viso con acqua fredda", descrizione: "Un gesto netto che abbassa la temperatura corporea e dona una sensazione di reset immediato.", isBreath: false },
  { id: 29, categoria: "anger", emoji: "ðŸ›¡ï¸", titolo: "Prenditi una pausa dalla discussione", descrizione: "Se stai litigando, chiedi un momento di stop. 'Ho bisogno di 10 minuti per calmarmi, poi ne parliamo'.", isBreath: false },
  { id: 30, categoria: "anger", emoji: "ðŸ”„", titolo: "Distrai la mente con un compito fisico", descrizione: "Riordina la scrivania, piega una maglietta, fai qualcosa di manuale per scaricare la tensione.", isBreath: false },

  // LONELY (Quando ti senti sola)
  { id: 31, categoria: "lonely", emoji: "ðŸ«‚", titolo: "Richiedi un abbraccio a Pisolo e Micio", descrizione: "Vai nella sezione abbracci e fatti stringere forte. Siamo qui per te, sempre.", isBreath: false },
  { id: 32, categoria: "lonely", emoji: "ðŸ“ž", titolo: "Ascolta un mio vocale", descrizione: "Riascolta un mio vecchio messaggio vocale in cui ridiamo insieme: la mia voce Ã¨ sempre con te.", isBreath: false },
  { id: 33, categoria: "lonely", emoji: "ðŸ“–", titolo: "Immergiti in una storia", descrizione: "Inizia a leggere un libro confortante o guarda la tua serie TV preferita per sentirti in compagnia.", isBreath: false },
  { id: 34, categoria: "lonely", emoji: "â¤ï¸", titolo: "Ricordati che sei amata", descrizione: "Non scordarlo mai: c'Ã¨ qualcuno al mondo che pensa a te e a cui sei preziosa, anche in questo momento.", isBreath: false },
  { id: 35, categoria: "lonely", emoji: "ðŸŽ¶", titolo: "Metti musica allegra di sottofondo", descrizione: "Riempi il silenzio della stanza con le note di una playlist felice o cantabile.", isBreath: false },
  { id: 36, categoria: "lonely", emoji: "â˜€ï¸", titolo: "Esci in un luogo affollato", descrizione: "Vai in un bar o siediti su una panchina al parco: stare tra la gente (senza dover parlare) puÃ² confortare.", isBreath: false }
];

const TIP_CATEGORIES = [
  { id: "ALL", label: "Tutte le situazioni", emoji: "âœ¨" },
  { id: "sleep", label: "Non riesci a dormire", emoji: "ðŸ’¤" },
  { id: "anxiety", label: "Ansia improvvisa", emoji: "ðŸ¦‹" },
  { id: "distance", label: "Vi mancate a distanza", emoji: "âœˆï¸" },
  { id: "heavy_day", label: "Giornata pesante", emoji: "ðŸ”‹" },
  { id: "anger", label: "Momento di rabbia", emoji: "ðŸŒªï¸" },
  { id: "lonely", label: "Ti senti sola", emoji: "ðŸ«‚" }
];
...
function shuffleComfortTips() {
  ...
  renderComfortTips();
  showToast("5 nuovi piccoli gesti estratti per te ðŸŒ¿âœ¨");
}
...
function toggleViewAllTips() {
  ...
  if (btn) {
    btn.innerHTML = isShowingAllTips ? `<span>âœ¨</span> Selezione (5)` : `<span>ðŸ“‹</span> Tutti (42)`;
  }
  renderComfortTips();
  showToast(isShowingAllTips ? "Visualizzazione completa di tutti i 42 gesti" : "Visualizzazione selezione di 5 gesti ðŸŒ¸");
}
...
function toggleTipCheck(tipId, event) {
  ...
  if (idx > -1) {
    ...
    showToast("Piccolo gesto compiuto con amore! ðŸŒ¸");
  }
...
function launchConfettiAnimation() {
  ...
  const symbols = ['ðŸ’–', 'âœ¨', 'ðŸŒ¸', 'ðŸ¦ˆ', 'ðŸ±', 'â­', 'ðŸŽˆ'];
...
function resetCheckedTips() {
  if (confirm("Vuoi azzerare tutti i gesti contrassegnati per ricominciare?")) {
    saveCheckedTipIds([]);
    renderComfortTips();
    showToast("Gesti azzerati: buon nuovo inizio! ðŸŒ¸");
  }
}
...
function updateTipsProgressBar(currentList, checkedIds) {
  ...
  textEl.innerHTML = `âœ¨ Gesti completati: <strong>${completed} su ${total}</strong> (${pct}%)`;
...
function renderComfortTips() {
  ...
  if (list.length === 0) {
    container.innerHTML = `<div style="text-align:center; padding:20px; font-size:13px; color:var(--warm-text-secondary);">Nessun gesto in questa categoria. Prova a toccare "Tutti" o "Nuovi gesti" ðŸŒ¸</div>`;
    return;
  }
    ...
    card.innerHTML = `
          ...
          <div class="tip-body-content">
            <div class="tip-item-title">
              ...
              <span class="tip-done-tag">FATTO âœ“</span>
            </div>
            <div class="tip-item-desc">${tip.descrizione}</div>
          </div>
          ${tip.isBreath ? '<span style="font-size:16px; color:var(--pastel-blue-accent); font-weight:bold; margin-left:4px;" title="Avvia respiro">â–¶</span>' : ''}
        `;
...
const BREATH_CYCLE = [
  { name: "Inspira lentamente...", desc: "Lascia entrare l'aria dal naso con calma, riempiendo il petto senza fretta.", seconds: 4, scale: 1.35, color: '#F8BBD0', icon: 'ðŸŒ¸', stepIdx: 0 },
  { name: "Trattieni...", desc: "Mantieni l'aria dentro dolcemente, sentendo la stabilitÃ  e la quiete.", seconds: 4, scale: 1.35, color: '#BBDEFB', icon: 'ðŸ«§', stepIdx: 1 },
  { name: "Espira con dolcezza...", desc: "Lascia uscire tutta l'aria dalla bocca, sciogliendo ogni tensione fisica.", seconds: 4, scale: 0.88, color: '#E1F5FE', icon: 'ðŸƒ', stepIdx: 2 },
  { name: "Pausa di riposo...", desc: "Resta un attimo in silenzio. Tutto va bene, sei al sicuro.", seconds: 2, scale: 0.88, color: '#FCE4EC', icon: 'â˜ï¸', stepIdx: 3 }
];
...
function openBreathingModal() {
  ...
  document.getElementById('breath-toggle-btn').innerHTML = 'â¸ï¸ Pausa';
...
function updatePrepUI() {
  ...
  if (icon) icon.textContent = 'ðŸ§˜â€â™€ï¸';
  ...
  if (prepPill) {
    ...
    prepPill.textContent = `â³ Inizio in ${prepSecondsLeft}s`;
  }
  for (let i = 0; i < 4; i++) {
...
function toggleBreathingRun() {
  ...
  document.getElementById('breath-toggle-btn').innerHTML = isBreathingActive ? 'â¸ï¸ Pausa' : 'â–¶ï¸ Riprendi';
}
...
function releaseThoughtsToSea() {
  ...
  for (let i = 0; i < 16; i++) {
    setTimeout(() => {
      ...
      b.textContent = ['ðŸ«§', 'ðŸŒŠ', 'ðŸ«§', 'âœ¨', 'â˜ï¸'][Math.floor(Math.random() * 5)];
  ...
  setTimeout(() => {
    ...
    showToast("I tuoi pensieri si sono sciolti tra le onde ðŸ«§");
  }, 350);
}
...
function openPartnerChat() {
  ...
  const waUrl = `https://wa.me/${cleanPhone}?text=${encodeURIComponent("Ciao amore mio â¤ï¸ Ho aperto l'app e volevo dirti che ho bisogno di te...")}`;
...
function editPartnerPhone() {
  ...
  if (input !== null) {
    ...
    showToast("Numero salvato con successo! â¤ï¸");
  }
}
...
function cleanGeneratedText(t) {
  if (!t) return '';
  return t.replace(/^["'â€œÂ«]+|["'â€Â»]+$/g, '').trim();
}
  ...
  for (const model of candidateModels) {
    try {
      ...
      if (data.error) {
        ...
        // Se la chiave Ã¨ errata (API_KEY_INVALID), fermati subito senza ciclare invano
...
function openApiKeyModal() {
  ...
  if (statusInd) {
    if (savedKey) {
      statusInd.textContent = 'âœ… Salvata e attiva';
...
async function testUserApiKey() {
  ...
  if (!key) {
    if (resultBox) {
      ...
      resultBox.textContent = 'Inserisci prima la tua chiave API prima di testarla ðŸŒ¸';
    }
    return;
  }

  if (btn) btn.textContent = 'â³ Verifica in corso...';
  if (resultBox) {
    resultBox.style.display = 'block';
  ...
  if (directResult.success) {
    ...
    resultBox.innerHTML = `âœ¨ <strong>Fantastico!</strong> La chiave Ã¨ valida e funziona alla perfezione! (${directResult.model}) ðŸ¦ˆðŸ±â¤ï¸`;
  } else {
    resultBox.style.background = '#FEE2E2';
    resultBox.style.color = '#991B1B';
    resultBox.style.border = '1px solid #FCA5A5';
    resultBox.textContent = 'âš ï¸ Errore: ' + (directResult.error || 'Verifica di aver copiato l\'intera chiave correttamente.');
  }

  if (btn) btn.textContent = 'ðŸ§ª Verifica Chiave';
}
...
function saveUserApiKey() {
  ...
  if (!key) {
    showToast("Inserisci la chiave prima di salvare ðŸŒ¸");
    return;
  }

  localStorage.setItem('user_gemini_api_key', key);
  updateApiKeyHeaderButton();
  showToast("Chiave salvata in modo sicuro nel tuo dispositivo! ðŸ”âœ¨");
  closeApiKeyModal();
}
...
function removeUserApiKey() {
  if (confirm("Vuoi davvero rimuovere la tua chiave API da questo dispositivo?")) {
    ...
    updateApiKeyHeaderButton();
    showToast("Chiave rimossa dal dispositivo ðŸ—‘ï¸");
  }
}
...
function updateApiKeyHeaderButton() {
  ...
  if (btn) {
    if (key) {
      ...
      if (txt) txt.textContent = 'IA Attiva âœ…';
    } else {
...
async function generateAiDedication(mood = 'dolce') {
  ...
  const userKey = getUserApiKey();
  const prompt = `Sei un'Intelligenza Artificiale dell'Amore e della Dolcezza, creata con tutto il cuore per dedicare parole sincere, uniche, dolci e profonde a una persona speciale (ispirata all'affetto incondizionato di Pisolo ðŸ¦ˆ uno squaletto tenero e Micio ðŸ± un gattino dolce).
...
Scrivi unicamente la frase d'amore in italiano, senza virgolette, senza asterischi, senza elenchi e senza spiegazioni (massimo 30-35 parole). Solo la frase.`;
  ...
  if (generatedText) {
    if (quoteEl) {
      ...
      setTimeout(() => {
        quoteEl.textContent = `â€œ${generatedText}â€`;
    ...
    if (signatureEl) {
      const moodNames = {
        'dolce': 'Con tutta la dolcezza di Pisolo ðŸ¦ˆ e Micio ðŸ±',
        'conforto': 'Un caldo abbraccio per te da Pisolo ðŸ¦ˆ, Micio ðŸ± e il tuo ragazzo',
        'incoraggiante': 'Pisolo ðŸ¦ˆ e Micio ðŸ± fanno sempre il tifo per te!',
        'buonanotte': 'Sogni d\'oro da Pisolo ðŸ¦ˆ e Micio ðŸ± ðŸŒ™',
        'buongiorno': 'Un buongiorno colmo d\'affetto da Pisolo ðŸ¦ˆ e Micio ðŸ± â˜€ï¸',
        'divertente': 'Una coccola buffa da Pisolo ðŸ¦ˆ e Micio ðŸ±'
      };
      signatureEl.textContent = `â€” ${moodNames[mood] || 'Pisolo ðŸ¦ˆ & Micio ðŸ± (con Gemini IA âœ¨)'}`;
    }

    if (badgeEl) {
      badgeEl.style.display = 'inline-flex';
      badgeEl.textContent = `âœ¨ Dedica creata da Gemini IA`;
    }

    // Aggiorna anche il testo nel modale abbraccio se aperto
    displayMessageInModal({
      quote: generatedText,
      author: "Pisolo ðŸ¦ˆ, Micio ðŸ± & il tuo ragazzo â¤ï¸"
    });

    showToast("Nuova dedica generata con affetto! ðŸŒ¸âœ¨");
  } else {
    shuffleFeaturedMessage();
    showToast("Ecco un pensiero speciale per te! ðŸ’•");
  }
...
async function generateAiTip() {
  const userKey = getUserApiKey();
  const prompt = `Sei un assistente affettuoso e premuroso, come Pisolo ðŸ¦ˆ (uno squaletto morbidissimo) e Micio ðŸ± (un gattino dolce).
Genera un singolo consiglio breve, pratico, caloroso e confortante per prendersi cura di sÃ© (tema o momento: "calma e coccole").
...
Niente titoli, niente introduzioni, niente elenchi puntati o asterischi. Solo il testo del consiglio.`;
  ...
  showToast("Pisolo e Micio stanno creando un consiglio per te... ðŸ¦ˆðŸ¾");
      ...
      aiCard.innerHTML = `
            ...
            <div class="tip-content-wrap">
              <div class="tip-pill-tag" style="background:#F3E8FF; color:#7E22CE; border:1px solid #E9D5FF;">
                <span>âœ¨</span> Consiglio IA Personalizzato
              </div>
              <div class="tip-text-body">
                ${generatedTip}
              </div>
              <div class="tip-mascot-author">
                <span>ðŸ¦ˆ</span> Pisolo &amp; Micio per te
              </div>
            </div>
          `;
      list.prepend(aiCard);
      aiCard.scrollIntoView({ behavior: 'smooth', block: 'center' });
      showToast("Consiglio IA aggiunto in cima alla lista! ðŸŒ¿âœ¨");
    }
  } else {
    shuffleComfortTips();
  }
...
window.addEventListener('DOMContentLoaded', () => {
  ...
  // Modale accoglienza: mostrato solo se giÃ  autenticato (altrimenti compare dopo il login)
  const overlay = document.getElementById('welcome-modal');
  if (overlay) {
