// Percorsi delle foto predefinite di Pisolo & Micio, salvate come file separati in /assets
  ...
  "gallery_1": "assets/img_10_376b8a82b05b.jpg",
  "gallery_2": "assets/img_11_d47deaf03d17.jpg",
  "gallery_3": "assets/img_12_fc58bab3bfed.jpg",
  "gallery_4": "assets/img_13_46d532346908.jpg",
  "gallery_5": "assets/img_10_376b8a82b05b.jpg",
  "gallery_6": "assets/img_12_fc58bab3bfed.jpg",
  "gallery_7": "assets/img_11_d47deaf03d17.jpg",
  "gallery_8": "assets/img_13_46d532346908.jpg"
};
    ...
    function resolveAsset(val, defaultAsset) {
      ...
      if (typeof val === 'string') {
        ...
        if (val.includes('gallery_default_1')) return EMBEDDED_ASSETS.gallery_1;
        if (val.includes('gallery_default_2')) return EMBEDDED_ASSETS.gallery_2;
        if (val.includes('gallery_default_3')) return EMBEDDED_ASSETS.gallery_3;
        if (val.includes('gallery_default_4')) return EMBEDDED_ASSETS.gallery_4;
        if (val.includes('gallery_default_5')) return EMBEDDED_ASSETS.gallery_5;
        if (val.includes('gallery_default_6')) return EMBEDDED_ASSETS.gallery_6;
        if (val.includes('gallery_default_7')) return EMBEDDED_ASSETS.gallery_7;
        if (val.includes('gallery_default_8')) return EMBEDDED_ASSETS.gallery_8;
      }
      return defaultAsset;
    }
    ...
    function setMascotPhoto(type, src, title, id) {
      if (type === 'shark') {
        localStorage.setItem('mascot_shark_selected_photo', src);
        if (id) {
          localStorage.setItem('mascot_shark_selected_id', id);
        } else {
          localStorage.setItem('mascot_shark_selected_id', 'custom');
        }
        if (title) localStorage.setItem('mascot_shark_selected_title', title);
        currentMascot = 'shark';
        localStorage.setItem('active_mascot_pref', 'shark');
      } else {
        localStorage.setItem('mascot_kitty_selected_photo', src);
        if (id) {
          localStorage.setItem('mascot_kitty_selected_id', id);
        } else {
          localStorage.setItem('mascot_kitty_selected_id', 'custom');
        }
        if (title) localStorage.setItem('mascot_kitty_selected_title', title);
        currentMascot = 'kitty';
        localStorage.setItem('active_mascot_pref', 'kitty');
      }

      updateMascotVisual();
      renderMascotPhotosGallery();
      renderModalMascotPhotos();
      showToast((type === 'shark' ? "Foto di Pisolo 🦈" : "Foto di Micio 🐱") + " aggiornata: " + (title || "nuovo ritratto") + "!");
    }

    function triggerMascotUpload(type) {
      const inputId = (type === 'kitty') ? 'upload-mascot-kitty-input' : 'upload-mascot-shark-input';
      const el = document.getElementById(inputId);
      if (el) el.click();
    }

    function handleMascotUpload(e, type) {
      const file = e.target.files && e.target.files[0];
      if (!file) return;
      compressImageFile(file, (b64) => {
        try {
          localStorage.setItem('custom_mascot_' + type + '_photo', b64);
          setMascotPhoto(type, b64, (type === 'shark' ? "Tuo Squaletto Personale 📸" : "Tuo Gattino Personale 📸"), 'custom');
        } catch (err) {
    ...
    function resetMascotPhoto(type) {
      if (type === 'shark') {
        setMascotPhoto('shark', EMBEDDED_ASSETS.shark_1, "Pisolo Classico 🦈", 'shark_1');
      } else {
        setMascotPhoto('kitty', EMBEDDED_ASSETS.kitty_1, "Micio Dolce 🐱", 'kitty_1');
      }
      showToast("Foto originale ripristinata! 🌸");
    }

    function switchMascotPhotoTab(tab) {
      currentMascotPhotoTab = tab;
      const allBtn = document.getElementById('tab-btn-all');
      const sharkBtn = document.getElementById('tab-btn-sharks');
      const kittyBtn = document.getElementById('tab-btn-kitties');
      [allBtn, sharkBtn, kittyBtn].forEach(b => b && b.classList.remove('active'));
      if (tab === 'all' && allBtn) allBtn.classList.add('active');
      else if (tab === 'shark' && sharkBtn) sharkBtn.classList.add('active');
      else if (tab === 'kitty' && kittyBtn) kittyBtn.classList.add('active');
      renderMascotPhotosGallery();
    }

    function switchModalPhotoTab(tab) {
      currentMascotPhotoTab = tab;
      const modalAllBtn = document.getElementById('modal-tab-all');
      const modalSharkBtn = document.getElementById('modal-tab-shark');
      const modalKittyBtn = document.getElementById('modal-tab-kitty');
      [modalAllBtn, modalSharkBtn, modalKittyBtn].forEach(b => b && b.classList.remove('active'));
      if (tab === 'all' && modalAllBtn) modalAllBtn.classList.add('active');
      else if (tab === 'shark' && modalSharkBtn) modalSharkBtn.classList.add('active');
      else if (tab === 'kitty' && modalKittyBtn) modalKittyBtn.classList.add('active');
      renderModalMascotPhotos();
    }

    function openMascotPhotoModal(defaultTab) {
      if (defaultTab) {
        currentMascotPhotoTab = defaultTab;
      } else {
        currentMascotPhotoTab = 'all';
      }
      switchModalPhotoTab(currentMascotPhotoTab);
      const modal = document.getElementById('modal-mascot-photos-backdrop');
      if (modal) modal.style.display = 'flex';
    }

    function closeMascotPhotoModal(e) {
      if (e && e.target !== document.getElementById('modal-mascot-photos-backdrop') && !e.target.classList.contains('modal-close-trigger')) {
        return;
      }
      const modal = document.getElementById('modal-mascot-photos-backdrop');
    ...
    function renderPhotoCardsHtml(type) {
      const activeSharkSrc = getActiveSharkPhoto();
      const activeSharkTitle = getActiveSharkTitle();
      const activeSharkId = getActiveSharkId();

      const activeKittySrc = getActiveKittyPhoto();
      const activeKittyTitle = getActiveKittyTitle();
      const activeKittyId = getActiveKittyId();

      const customShark = localStorage.getItem('custom_mascot_shark_photo');
      const customKitty = localStorage.getItem('custom_mascot_kitty_photo');

      let html = `
        <div class="mascot-current-banner">
          <div style="display:flex; align-items:center; gap:10px;">
            <img class="mascot-current-thumb shark" src="${activeSharkSrc}" alt="${activeSharkTitle}" title="Pisolo 🦈">
            <img class="mascot-current-thumb kitty" src="${activeKittySrc}" alt="${activeKittyTitle}" title="Micio 🐱">
          </div>
          <div class="mascot-current-info">
            <div class="mascot-current-title">Pisolo 🦈 & Micio 🐱</div>
            <div class="mascot-current-sub">Foto attive sempre alternate per starti vicino</div>
          </div>
          <div class="mascot-banner-actions">
            <button type="button" class="mascot-btn-upload-small" onclick="triggerMascotUpload('${type === 'kitty' ? 'kitty' : 'shark'}')" title="Carica una tua foto">
              <span>📷</span> Carica
            </button>
            <button type="button" class="mascot-btn-reset-small" onclick="resetMascotPhoto('shark'); resetMascotPhoto('kitty');" title="Ripristina originali">
              🔄 Reset
            </button>
          </div>
        </div>
        <div class="mascot-photo-grid">
      `;

      let listToRender = [];
      if (type === 'all') {
        listToRender = ALTERNATING_MASCOT_PRESETS;
      } else if (type === 'shark') {
        listToRender = SHARK_PHOTOS_PRESETS;
      } else {
        listToRender = KITTY_PHOTOS_PRESETS;
      }

      listToRender.forEach(p => {
        const isShark = (p.type === 'shark');
        const activeSrc = isShark ? activeSharkSrc : activeKittySrc;
        const activeId = isShark ? activeSharkId : activeKittyId;
        const photoSrc = EMBEDDED_ASSETS[p.id] || activeSrc;

        const isSelected = (p.id === activeId) || (photoSrc === activeSrc && activeId !== 'custom');
        const selectedClass = isSelected ? (isShark ? 'selected-shark' : 'selected-kitty') : '';
        const pillClass = isShark ? 'shark' : 'kitty';

        html += `
          <div class="mascot-photo-card ${selectedClass}" onclick="setMascotPhoto('${p.type}', '${photoSrc}', '${p.title}', '${p.id}')" title="Usa ${p.title}">
            <div class="mascot-photo-img-wrap">
              <img src="${photoSrc}" alt="${p.title}" loading="eager">
              ${isSelected ? `<div class="mascot-active-pill ${pillClass}">✓ In uso</div>` : ''}
              <div class="mascot-tag-pill" style="${isShark ? '' : 'background:var(--pastel-pink); color:#fff;'}">${p.tag}</div>
            </div>
            <div class="mascot-photo-caption">
              <div class="mascot-photo-name">${p.title}</div>
              <div class="mascot-photo-desc">${p.desc}</div>
            </div>
          </div>
        `;
      });

      // Foto personalizzate se caricate
      if (customShark && (type === 'all' || type === 'shark')) {
        const isSelected = (activeSharkId === 'custom');
        html += `
          <div class="mascot-photo-card ${isSelected ? 'selected-shark' : ''}" onclick="setMascotPhoto('shark', '${customShark}', 'Tuo Squaletto Personale 📸', 'custom')">
            <div class="mascot-photo-img-wrap">
              <img src="${customShark}" alt="Foto Squaletto personalizzata">
              ${isSelected ? `<div class="mascot-active-pill shark">✓ In uso</div>` : ''}
              <div class="mascot-tag-pill" style="background:var(--sky-blue); color:#fff;">Tuo Squaletto 🦈</div>
            </div>
            <div class="mascot-photo-caption">
              <div class="mascot-photo-name">Squaletto Personale</div>
              <div class="mascot-photo-desc">Caricata da te per Pisolo</div>
            </div>
          </div>
        `;
      }

      if (customKitty && (type === 'all' || type === 'kitty')) {
        const isSelected = (activeKittyId === 'custom');
        html += `
          <div class="mascot-photo-card ${isSelected ? 'selected-kitty' : ''}" onclick="setMascotPhoto('kitty', '${customKitty}', 'Tuo Gattino Personale 📸', 'custom')">
            <div class="mascot-photo-img-wrap">
              <img src="${customKitty}" alt="Foto Gattino personalizzata">
              ${isSelected ? `<div class="mascot-active-pill kitty">✓ In uso</div>` : ''}
              <div class="mascot-tag-pill" style="background:var(--deep-rose); color:#fff;">Tuo Gattino 🐱</div>
            </div>
            <div class="mascot-photo-caption">
              <div class="mascot-photo-name">Gattino Personale</div>
              <div class="mascot-photo-desc">Caricata da te per Micio</div>
            </div>
          </div>
        `;
      }
    ...
    function renderMascotPhotosGallery() {
      const area = document.getElementById('mascot-gallery-content-area');
      if (area) {
        area.innerHTML = renderPhotoCardsHtml(currentMascotPhotoTab);
      }
    }

    function renderModalMascotPhotos() {
      const area = document.getElementById('modal-mascot-content-area');
      if (area) {
        area.innerHTML = renderPhotoCardsHtml(currentMascotPhotoTab);
      }
    }
    ...
    function renderCarousel() {
      ...
      SUPPORT_MESSAGES.forEach((msg, idx) => {
        // Alternanza rigorosa: ogni card alterna squaletti e gattini
        const photoObj = getAlternatingPhoto(idx);
        const photoSrc = photoObj.src;
        const mascotBadge = photoObj.badge;

        const slide = document.createElement('div');
        slide.className = 'carousel-slide';
        slide.innerHTML = `
          <div class="carousel-photo-holder" onclick="openThoughtDetail(${idx})" title="Tocca per ingrandire foto e dedica">
            <img src="${photoSrc}" alt="${msg.title || msg.titolo}" onerror="this.src='${photoObj.type === 'shark' ? EMBEDDED_ASSETS.shark_1 : EMBEDDED_ASSETS.kitty_1}'">
            <div class="carousel-badge">${mascotBadge}</div>
            <div class="carousel-photo-zoom-icon">🔍 Ingrandisci</div>
          </div>
          <div class="carousel-content">
            <div style="display:inline-block; font-size:11px; font-weight:700; color:var(--deep-rose); margin-bottom:4px; text-transform:uppercase; letter-spacing:0.5px;">${msg.tag}</div>
            <div class="carousel-title">${msg.title || msg.titolo}</div>
            <div class="carousel-text">“${msg.testo}”</div>
            <div class="carousel-author">${msg.autore || msg.firma}</div>
          </div>
        `;
        track.appendChild(slide);

        // Indicatori dots per navigazione
        if (dotsContainer && idx % 4 === 0) {
          const dot = document.createElement('div');
          dot.className = `carousel-dot ${Math.floor(currentSlideIndex / 4) === Math.floor(idx / 4) ? 'active' : ''}`;
    ...
    function goToSlide(idx) {
      currentSlideIndex = (idx + SUPPORT_MESSAGES.length) % SUPPORT_MESSAGES.length;
      updateCarouselPosition();
    }
    ...
    function updateCarouselPosition() {
      ...
      if (counter) {
        counter.textContent = `${currentSlideIndex + 1} di ${SUPPORT_MESSAGES.length}`;
      }
      const activeDotIdx = Math.floor(currentSlideIndex / 4);
      dots.forEach((d, i) => {
        d.classList.toggle('active', i === activeDotIdx);
      });
    }

    let activeSelectedPhotoSrc = '';
    function openThoughtDetail(idx) {
      const msg = SUPPORT_MESSAGES[idx];
      const photoObj = getAlternatingPhoto(idx);
      activeSelectedPhotoSrc = photoObj.src;

      const lightboxImg = document.getElementById('lightbox-img-el');
      const lightboxTag = document.getElementById('lightbox-title-tag');
      const lightboxTitle = document.getElementById('lightbox-caption-el');
      const lightboxDate = document.getElementById('lightbox-date-el');

      if (lightboxImg) lightboxImg.src = photoObj.src;
      if (lightboxTag) lightboxTag.textContent = `${photoObj.badge} • ${msg.tag}`;
      if (lightboxTitle) lightboxTitle.textContent = msg.title || msg.titolo;
      if (lightboxDate) lightboxDate.textContent = `“${msg.testo}” — ${msg.autore || msg.firma}`;

      const backdrop = document.getElementById('modal-lightbox-backdrop');
      if (backdrop) backdrop.style.display = 'flex';
    }

    function closePhotoLightbox(e) {
      if (e && e.target !== e.currentTarget) return;
      document.getElementById('modal-lightbox-backdrop').style.display = 'none';
    }

    function setLightboxAsFeatured() {
      if (!activeSelectedPhotoSrc) return;
      document.getElementById('featured-photo-img').src = activeSelectedPhotoSrc;
      document.getElementById('modal-photo-el').src = activeSelectedPhotoSrc;
      try {
        localStorage.setItem('featured_hero_photo', activeSelectedPhotoSrc);
      } catch (e) {}
      document.getElementById('modal-lightbox-backdrop').style.display = 'none';
      showToast("Foto impostata come copertina principale! ⭐");
    }
    ...
    window.addEventListener('beforeinstallprompt', (e) => {
      e.preventDefault();
      deferredPwaPrompt = e;
      const btn = document.getElementById('header-btn-install');
    ...
    window.addEventListener('appinstalled', () => {
      ...
      showToast("App installata con successo sulla schermata Home! 💖");
    });
      ...
      if (deferredPwaPrompt) {
        ...
        deferredPwaPrompt.userChoice.then((choiceResult) => {
          if (choiceResult.outcome === 'accepted') {
            showToast("Installazione avviata! Benvenuta a casa 🌸");
          }
          deferredPwaPrompt = null;
        });
      } else {
        showToast("Tocca il menu del browser e seleziona 'Aggiungi a schermata Home' 📲");
      }
    }
