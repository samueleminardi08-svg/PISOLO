// Percorsi delle foto predefinite di Pisolo & Micio, salvate come file separati in /assets
  ...
  "shark_1": "assets/mascots/img_01_0a586335395e.jpg",
  "shark_2": "assets/mascots/img_03_fa6339c60a33.jpg",
  "shark_3": "assets/mascots/img_04_474c0282a71d.jpg",
  "shark_4": "assets/mascots/img_05_44e40f9a0c35.jpg",
  "kitty_1": "assets/mascots/img_02_80bf04a43bbc.jpg",
  "kitty_2": "assets/mascots/img_06_7d757a1dd8a1.jpg",
  "kitty_3": "assets/mascots/img_07_96256b3c039b.jpg",
  "kitty_4": "assets/mascots/img_08_2c3322d80209.jpg",
  "photo_featured": "assets/gallery/img_09_217ed04bd5bb.jpg",
  "gallery_1": "assets/gallery/img_10_376b8a82b05b.jpg",
  "gallery_2": "assets/gallery/img_11_d47deaf03d17.jpg",
  "gallery_3": "assets/gallery/img_12_fc58bab3bfed.jpg",
  "gallery_4": "assets/gallery/img_13_46d532346908.jpg",
  "gallery_5": "assets/gallery/img_10_376b8a82b05b.jpg",
  "gallery_6": "assets/gallery/img_12_fc58bab3bfed.jpg",
  "gallery_7": "assets/gallery/img_11_d47deaf03d17.jpg",
  "gallery_8": "assets/gallery/img_13_46d532346908.jpg"
};
    ...
    function logoutApp() {
      ...
      localStorage.removeItem('pisolo_partner_phone');
      localStorage.removeItem('pisolo_checked_tips');
      sessionStorage.removeItem('pisolo_session_tip_ids');
      location.reload();
    }
    ...
    async function openHugModal(category = null) {
      ...
      if (userKey && !isAiHugGenerating) {
        ...
        // Se c'è una dedica pronta in cache, mostrala subito (nessun loader)
        if (prefetchedHug) {
          msg = prefetchedHug;
          prefetchedHug = null;
          prefetchHugPromise = null;
        } else {
          // Mostra il loader delicato solo se dobbiamo aspettare
          ...
          // Attendi prefetch in corso, oppure genera al volo
          msg = await getHugFromCacheOrGenerate(activeHugCategory);
        }
      ...
      // Mostra foto e dedica
      if (modalPhoto && msg.image) {
        modalPhoto.src = msg.image;
      }
      displayMessageInModal(msg);
      displayMessageOnCard(msg);

      // 🚀 Avvia subito il prefetch della PROSSIMA dedica in background
    ...
    async function cycleNextHugMessage() {
      ...
      if (userKey && !isAiHugGenerating) {
        ...
        // Se c'è una dedica pronta in cache, mostrala subito
        if (prefetchedHug) {
          msg = prefetchedHug;
          prefetchedHug = null;
          prefetchHugPromise = null;
        } else {
          // Mostra il loader solo se dobbiamo aspettare
    ...
    // ============================================================
    const TRANQUIL_BACKGROUNDS = [{"id": "spiaggia-serena", "name": "Spiaggia Serena", "emoji": "🏖️", "image": "assets/landscapes/img_14_71f7b0643166.jpg", "place": "Spiaggia calma e mare turchese trasparente", "themeColor": "#EBF5FB"}, {"id": "boschetto-luce", "name": "Boschetto di Luce", "emoji": "🌲", "image": "assets/landscapes/img_15_938af34ab049.jpg", "place": "Bosco silenzioso con raggi di sole gentili", "themeColor": "#EAF8F1"}, {"id": "lago-quieto", "name": "Lago Quieto", "emoji": "🏞️", "image": "assets/landscapes/img_16_599c09a8ed3b.jpg", "place": "Lago alpino immobile e trasparente all'aurora", "themeColor": "#F2EEF8"}, {"id": "campi-lavanda", "name": "Campi di Lavanda", "emoji": "🪻", "image": "assets/landscapes/img_17_016e937ee8b6.jpg", "place": "Distesa fiorita provenzale al tramonto", "themeColor": "#F5EFFB"}, {"id": "giardino-zen", "name": "Giardino Zen", "emoji": "🌸", "image": "assets/landscapes/img_18_c11de8ca4c11.jpg", "place": "Fiori di ciliegio, specchio d'acqua e quiete", "themeColor": "#FDF0F4"}, {"id": "tramonto-mare", "name": "Tramonto sul Mare", "emoji": "🌅", "image": "assets/landscapes/img_19_2ad37fb1fbd2.jpg", "place": "Spiaggia dorata soffice e orizzonte sereno", "themeColor": "#FFF3E8"}, {"id": "prato-fiorito", "name": "Prato Fiorito", "emoji": "🌼", "image": "assets/landscapes/img_20_a1294a4cb05c.jpg", "place": "Dolci colline verdi con margherite campestri", "themeColor": "#F4FAF2"}, {"id": "cascata-segreta", "name": "Cascata Segreta", "emoji": "💧", "image": "assets/landscapes/img_21_238c3b53e343.jpg", "place": "Ruscello limpido tra sassi levigati e muschio", "themeColor": "#EDF8F7"}, {"id": "colline-aurora", "name": "Colline all'Aurora", "emoji": "🌄", "image": "assets/landscapes/img_22_24b7db2b4e85.jpg", "place": "Vallata verde con foschia dorata mattutina", "themeColor": "#F7FBF4"}, {"id": "fiume-salici", "name": "Fiume dei Salici", "emoji": "🌿", "image": "assets/landscapes/img_23_e58588a5599c.jpg", "place": "Ninfee e salici piangenti sull'acqua cheta", "themeColor": "#EEF7F5"}];
    ...
    function initSessionTips(forceNew = false) {
      ...
      try {
        ...
        localStorage.removeItem('pisolo_checked_tips'); // Clear old checks to prevent fake celebration
      } catch (e) {}
    }
    ...
    function renderComfortTips() {
      ...
      list.forEach(tip => {
        ...
        let cardClasses = `tip-card-item ${tip.isBreath ? 'is-breath-tip' : ''} ${isChecked ? 'is-completed' : ''}`;
        if (tip.isAi && !isChecked) cardClasses += ' active-card';
        card.className = cardClasses;
        
        if (tip.isAi) {
          card.style.borderColor = '#C084FC';
          card.style.background = 'linear-gradient(135deg, rgba(250,245,255,0.95), rgba(253,242,248,0.95))';
        }
        ...
        card.innerHTML = `
          ...
          <div class="tip-icon-avatar" style="background: ${tip.isAi ? 'var(--pastel-purple-container, #E9D5FF)' : (tip.isBreath ? 'var(--pastel-blue-container)' : 'var(--pastel-pink-container)')}">
            ${tip.emoji}
          </div>
      ...
      breathTimerInterval = setInterval(() => {
        ...
        if (isPrepCountdown) {
          ...
          if (totalSessionSeconds <= 0) {
            closeBreathingModal();
            showToast("Bravissima, hai completato 60 secondi di respirazione! 🌸");
            return;
          }
          secondsLeft--;
          if (secondsLeft <= 0) {
    ...
    async function callGeminiDirect(apiKey, prompt, systemInstruction = null, maxTokens = 200) {
      ...
      try { lastWorkingModel = sessionStorage.getItem('pisolo_last_gemini_model'); } catch(e) {}

      // Se un modello ha già funzionato in questa sessione, provalo per primo
      if (lastWorkingModel && !candidateModels.includes(lastWorkingModel)) {
        candidateModels.unshift(lastWorkingModel);
      } else if (lastWorkingModel) {
        const idx = candidateModels.indexOf(lastWorkingModel);
        if (idx > 0) {
      ...
      for (const model of candidateModels) {
        try {
          ...
          if (res.ok && data.candidates && data.candidates.length > 0 && data.candidates[0].content && data.candidates[0].content.parts && data.candidates[0].content.parts.length > 0) {
            ...
            if (text) {
              // Ricorda il modello funzionante per le prossime chiamate
              try { sessionStorage.setItem('pisolo_last_gemini_model', model); } catch(e) {}
              return { success: true, text: text.trim(), model };
            }
          }
          if (data.error) {
            lastErr = data.error.message || JSON.stringify(data.error);
            // Chiave errata → fermati subito
    ...
    async function generateAiDedication(mood = 'dolce') {
      ...
      const systemInstruction = `Sei l'anima tenera e protettiva di questa applicazione, creata da un fidanzato amorevole per la sua ragazza (che ogni tanto puoi chiamare affettuosamente "Olaf" o "piccola mia"). Rappresenti l'affetto del ragazzo, unito alla tenerezza incondizionata di due mascotte: Pisolo (uno squaletto peluche azzurro che fa da scudo protettivo) e Micio (un gattino morbido che dona calore). Il tuo tono è profondamente caldo, empatico, rassicurante e colmo d'amore. Valida sempre i sentimenti della ragazza. Rivolgiti a lei con dolcezza infinita.`;
      const prompt = `Genera una singola frase d'amore, di vicinanza o di conforto inedita e romanticissima (tono o atmosfera: "${mood}"). Scrivi unicamente la frase in italiano, senza virgolette, senza asterischi, senza elenchi e senza spiegazioni (massimo 30-35 parole). Solo la frase.`;

      let generatedText = '';

      if (userKey) {
        const directRes = await callGeminiDirect(userKey, prompt, systemInstruction, 120);
    ...
    function openAutonomousLinkModal() {
      const urlToCopy = typeof AUTONOMOUS_PERMANENT_URL !== 'undefined' && AUTONOMOUS_PERMANENT_URL 
        ? AUTONOMOUS_PERMANENT_URL 
        : window.location.href;
      
      if (navigator.clipboard && navigator.clipboard.writeText) {
        navigator.clipboard.writeText(urlToCopy).then(() => {
          showToast("Link dell'app copiato negli appunti! 🔗");
        }).catch(() => {
          prompt("Copia questo link:", urlToCopy);
        });
      } else {
        prompt("Copia questo link:", urlToCopy);
      }
